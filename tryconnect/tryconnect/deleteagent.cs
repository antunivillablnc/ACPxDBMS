...
namespace tryconnect
{
    public partial class deleteagent : Form
    {
        ...
        private void deletebttnagent_Click(object sender, EventArgs e)
        {
            if (agentidtxtdelete.Text != "")
            {
                cmd = new MySqlCommand($"delete from sample.agents where agentID='{agentidtxtdelete.Text}'", con);
                con.Open();
                cmd.Parameters.AddWithValue("@agentID", agentidtxtdelete.Text);
                cmd.ExecuteNonQuery();
                con.Close();
        ...
        {
            agentidtxtdelete.Text = dataGridView1.Rows[e.RowIndex].Cells[0].Value.ToString();
        }

        private void returnpropertyedit_Click(object sender, EventArgs e)
        {
            this.Close();
            agents returnagentsdeletefrm = new agents();
            returnagentsdeletefrm.ShowDialog();
        }
    }
}
